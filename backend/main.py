...
from backend import weather_service, prediction_service
...
def startup_event():
    ...
    except prediction_service.ModelLoadError as exc:
...
def predict(
    ...
    if not prediction_service.models_ready():
        raise HTTPException(status_code=500, detail="Prediction models unavailable")

    try:
        hourly_df = weather_service.fetch_hourly_weather(latitude, longitude)
    ...
    try:
        result = prediction_service.build_forecast(latitude, longitude, hourly_df)
    except prediction_service.ModelLoadError as exc:
